...
\mytext[current][model]{.62\blockwidth}{anchor = north west,
  ...
  \begin{equation*}
    \begin{array}{ll@{\quad}l}
      ...
      \\
      \smallskip
      & \ds \lb_{pt}^i \open_p^i \le \rate_{pt}^i\va\quantity_{pt}^i \le \ub_{pt}^i \open_p^i & \forall t, \forall p, \forall i
      \\
      \smallskip
      & \red{\avar_{1-\servicelvl}\bracket{\va\inventory_t^i} \le 0} & \forall t, \forall i
      \\
      \smallskip
      & \ds \va\quantity_{pt}^i\ \mbox{is}\ \Sfield{\bracket{\va\demand_{\tau}^j}_{j, \tau\le t}}\mbox{--measurable} & \forall t, \forall p, \forall i
      \\
      \smallskip
      & \ds \open_p^i \in \crbracket{0,1} & \forall p, \forall i
    \end{array}
  \end{equation*}
}
...
\mytext[legend.south]{\blockwidth}{anchor = north,
  ...
  \begin{center}
  \input{images/Mel_solver_vs_heuristic.tikz}
  \end{center}
}
